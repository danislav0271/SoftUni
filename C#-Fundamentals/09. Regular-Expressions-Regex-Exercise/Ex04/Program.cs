...
namespace Ex04
{
    internal class Program
    {
        static void Main(string[] args)
        {
            ...
            string pattern = @"\@(?<planet>[A-Za-z]+)[^\@\-\!\:\>]*?\:(?<population>\d+)[^\@\-\!\:\>]*?\!(?<type>(A|D){1})\![^\@\-\!\:\>]*?\-\>(?<count>\d+)";
            ...
            for (int i = 0; i < n; i++)
            {
                ...
                if (orderInfo.Success)
                {
                    ...
                    string attType = orderInfo.Groups["type"].Value;
        ...
        static string DecryptMessage(string encryptedMessage)
        {
            ...
            int decryptionKey = GetDecryptionKet(encryptedMessage);

            foreach (char item in encryptedMessage)
            {
                char decryptedCh = (char)(item - decryptionKey);
        ...
        static int GetDecryptionKet(string encryptedMessage)
        {
